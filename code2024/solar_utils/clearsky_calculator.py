...
class ClearskyCalculator:
    ...
    informed by the monthly TL values derived from the research presented in the Janjai paper,
    ...
    _metadata_dir = os.path.join(_module_dir, "configs")
    
    # Read all relevant metadata
    _gridtiff_metadata_df = pd.read_csv(os.path.join(_metadata_dir,'plant_metadata.csv'))
    ...
    _JanJai_metadata_df = pd.read_csv(os.path.join(_metadata_dir,'JanJaiAngstromTurbidity.csv'))
    ...
    def __init__(self, lat, long, defined_TL=None, defined_alt=None, monthly_TL=None):
        """
        ...
        defined_TL (float, optional): User-defined fixed Linke turbidity coefficient for all months. Default is None.
        defined_alt (float, optional): User-defined altitude of the location in meters. Default is None.
        monthly_TL (np.ndarray, optional): User-defined array of monthly Linke turbidity coefficients. Default is None.
        ...
        defined_TL (float or None): User-defined fixed TL value.
        defined_alt (float or None): User-defined altitude value.
        monthly_TL (np.ndarray or None): User-defined array of monthly TL values.
        """
        self.lat = lat
        self.long = long
    ...
    def get_nearest_measurement_metadata(self):
        ...
        Returns:
        tuple: Nearest altitude and estimated TL value from the metadata.
        """
    ...
    def get_nearest_JanJai_metadata(self):
        """
        Get the nearest Janjai metadata based on the user's location.
        ...
        nearest_index = np.argmin(dist_arr)
        chosen_beta_df = ClearskyCalculator._JanJai_metadata_df.loc[nearest_index, ['Jan', 'Feb', 'Mar', 'Apr', 'May', 
                                                                                    'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 
                                                                                    'Nov', 'Dec']]
    ...
    def cal_cos_zna(self, start_date, end_date, freq='15min'):
        """
        ...
        freq (str): Frequency of the calculations. Default is '15min'.

        Returns:
        pd.DataFrame: DataFrame containing the datetime index and cosine of the solar zenith angle.
        """
        phi = self.lat
        ...
        cos_zna = np.sin(delta * np.pi / 180) * np.sin(phi * np.pi / 180) + np.cos(delta * np.pi / 180) * \
                  np.cos(phi * np.pi / 180) * np.cos(HRA * np.pi / 180)
    ...
    def cal_dairmass(self, start_date, end_date, freq='15min', cos_zna_df=None):
        """
        Calculate airmass.
        ...
        end_date (str): End date in the format 'YYYY-MM-DD'.
        freq (str): Frequency of the calculations. Default is '15min'.
        cos_zna_df (pd.DataFrame, optional): DataFrame containing the cosine of the solar zenith angle. Default is None.

        Returns:
        pd.DataFrame: DataFrame containing the datetime index and airmass values.
        """
    ...
    def cal_clearsky_irradiance(self, start_date, end_date, freq='15min', choice='monthly_estimate', cos_zna_df=None, AM_df=None):
        """
        ...
        end_date (str): End date in the format 'YYYY-MM-DD'.
        freq (str): Frequency of the calculations. Default is '15min'.
        choice (str): Choice for TL values ('monthly_estimate', 'estimate', 'JanJai', or 'defined'). Default is 'monthly_estimate'.
        cos_zna_df (pd.DataFrame, optional): DataFrame containing the cosine of the solar zenith angle. Default is None.
        AM_df (pd.DataFrame, optional): DataFrame containing the airmass values. Default is None.

        Returns:
        pd.DataFrame: DataFrame containing the datetime index and clear-sky irradiance values.
        """
        if cos_zna_df is None:
            cos_zna_df = self.cal_cos_zna(start_date, end_date, freq)
        if AM_df is None:
    ...
    def get_solar_info(self, start_date, end_date, freq='15min', choice='monthly_estimate'):
        """
        ...
        Returns:
        pd.DataFrame: DataFrame containing the datetime index and all calculated information (cosine of the solar zenith angle, 
                      airmass, and clear-sky irradiance).
        """
        cos_zna_df = self.cal_cos_zna(start_date, end_date, freq)
        AM_df = self.cal_dairmass(start_date, end_date, freq, cos_zna_df)
